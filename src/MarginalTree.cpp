...
//cout << "leaf id: " << idNew << " for node: " << nodeCurrent << endl;
	}
...
void ReadinMarginalTreesNewickWLenString2( const string &strNewick, int numLeaves, MarginalTree &treeOut )
{
    // Jan 10, 2023. YW: trying to fix an error in leaf labels
//YW_ASSERT_INFO(pTMapper != NULL, "Stop here2");
    // mark the change
    // NOTE: RETURN TRUE IF NO LABEL ADJUSTMENT IS DONE
    // RETURN FALSE IF WE SWITCHED LABEL BY DECREASING BY ONE
...
//cout << "Set number of leaves of marginal tree to: " << numLeaves << endl;
    ...
    int nodeIntNext = numTotNodes-1;
    string strNewickUse = strNewick;
    UpdateMTreeWithNWString( treeOut, leafNext, nodeIntNext, strNewickUse, NULL );
//cout << "Immediate after UpdateMTreeWithNWString: treeOut: \n";
//treeOut.Dump();

    // finally prepare marginal tree for query
    treeOut.BuildDescendantInfo();
//cout << "ReadinMarginalTreesNewickWLenString2: newick string = \n" << treeOut.GetNewick() << endl;
}
...
void MarginalTree :: BuildDescendantInfo2()
{
    // Note, this only works for binary tree!!!!!
    listLeftDescs.clear();
    listRightDescs.clear();
...
//cout << "BuildDescendantInfo: numNodes: " << numNodes << endl;
    ...
    // get ordered nodes
    vector<int> listOrderedNodes;
    GetAllNodesBottomUpFrom(GetRoot(), listOrderedNodes);
    
    // handle other cases
    for( int index=0; index<numNodes; ++index )
    {
        int i = listOrderedNodes[index];
        int p = GetParent(i);
//cout << "paret of " << i << " is " << p << endl;
        if( p < 0 )
        {
...
int MarginalTree :: GetLeftDescendant( int node ) const
{
    YW_ASSERT_INFO((int)listLeftDescs.size() == GetTotNodesNum() && 
        (int)listRightDescs.size() == GetTotNodesNum(), "descendant info not set");
...
void MarginalTree :: SetLeftDescendant(int node, int nodeLeftDesc)
{
    YW_ASSERT_INFO((int)listLeftDescs.size() == GetTotNodesNum() &&
        (int)listRightDescs.size() == GetTotNodesNum(), "descendant info not set");
    listLeftDescs[node] = nodeLeftDesc;
}
void MarginalTree :: SetRightDescendant(int node, int nodeRightDesc)
{
    YW_ASSERT_INFO((int)listLeftDescs.size() == GetTotNodesNum() &&
        (int)listRightDescs.size() == GetTotNodesNum(), "descendant info not set");
    listRightDescs[node] = nodeRightDesc;
}
...
void MarginalTree :: PerformSPR(int nodeSPRSrc, int nodeSPRDest, bool fReOrdering)
{
    // perform tree rearrangement
    // cannot have root be involved in SPR
    // if fReOrdering = true, re-initialze the tree positions of all nodes
    if( nodeSPRSrc == GetRoot() || nodeSPRDest == GetRoot() )
    {
        cout << "WARNING: cannot SPR invoves root\n";
        return;
    }
    
    int nodeSPRSrcSib = GetSibling(nodeSPRSrc);
    int nodeSPRSrcPar = GetParent(nodeSPRSrc);
    int nodeSPRSrcParPar = -1;
    if( nodeSPRSrcPar != GetRoot() )
    {
        nodeSPRSrcParPar = GetParent(nodeSPRSrcPar);
    }
    int nodeSPRDestPar = GetParent(nodeSPRDest);
    if( nodeSPRSrcSib == nodeSPRDest || nodeSPRSrcPar == nodeSPRDest )
    {
        // donot allow SPR src and dest be siblings already
        cout << "WARNING: cannot have two siblings be SPR\n";
        cout << "Current tree: " << GetNewickSorted(false) << endl;
        cout << "nodeSPRSrc:" << nodeSPRSrc << ", nodeSPRDest: " << nodeSPRDest << ", tree:";
        this->Dump();
        
        return;
    }
    SetParent( nodeSPRSrcSib, GetParent(nodeSPRSrcPar), false );
    SetParent( nodeSPRDest, nodeSPRSrcPar, false );
    SetParent( nodeSPRSrcPar, nodeSPRDestPar, false);
    
    if( fReOrdering == false )
    {
        // also need to change descendent info since we are not rebuilding
        // change src'par'par if needed
        if( nodeSPRSrcParPar >= 0 )
        {
            // see left or right to change
            int nppleft = GetLeftDescendant(nodeSPRSrcParPar);
            int nppright = GetRightDescendant(nodeSPRSrcParPar);
            if(nppleft == nodeSPRSrcPar)
            {
                SetLeftDescendant(nodeSPRSrcParPar, nodeSPRSrcSib);
            }
            else
            {
                YW_ASSERT_INFO(nppright == nodeSPRSrcPar, "Must be equal here");
                SetRightDescendant(nodeSPRSrcParPar, nodeSPRSrcSib);
            }
        }
        // change the updated nodesrcpar
        if(GetLeftDescendant(nodeSPRSrcPar) == nodeSPRSrc)
        {
            SetRightDescendant(nodeSPRSrcPar, nodeSPRDest);
        }
        else
        {
            YW_ASSERT_INFO(GetRightDescendant(nodeSPRSrcPar) == nodeSPRSrc, "Must be equal here1");
            SetLeftDescendant(nodeSPRSrcPar, nodeSPRDest);
        }
        // update dest'par
        if( GetLeftDescendant(nodeSPRDestPar) == nodeSPRDest )
        {
            SetLeftDescendant(nodeSPRDestPar, nodeSPRSrcPar);
        }
        else
        {
            YW_ASSERT_INFO( GetRightDescendant(nodeSPRDestPar) == nodeSPRDest, "Must be equal here2");
            SetRightDescendant(nodeSPRDestPar, nodeSPRSrcPar);
        }
    ...
    // rearrange
    RearrangeParIncOrder();
    
    // re-init descendents
    BuildDescendantInfo();
}

void MarginalTree :: PerformSPRWithDescInfo( int nodeSPRSrc, int nodeSPRDest, const vector<vector<int> > &descNodesInfo  )
{
    // perform tree rearrangement
    // cannot have root be involved in SPR
    // if fReOrdering = true, re-initialze the tree positions of all nodes
    ...
    int nodeSPRSrcSib = GetSiblingFromDescInfo(nodeSPRSrc, descNodesInfo);
    int nodeSPRSrcPar = GetParent(nodeSPRSrc);
    //int nodeSPRSrcParPar = -1;
    //if( nodeSPRSrcPar != GetRoot() )
    //{
    ...
    {
        // donot allow SPR src and dest be siblings already
        cout << "WARNING: cannot have two siblings be SPR2\n";
        return;
    }
    SetParent( nodeSPRSrcSib, GetParent(nodeSPRSrcPar), false );
    SetParent( nodeSPRDest, nodeSPRSrcPar, false );
...
void MarginalTree :: RerootSubtree(int nodeST, int nodeSTNewRoot)
{
//cout << "Reroot tree: nodeST:" << nodeST << ", nodeSTNewRoot: " << nodeSTNewRoot << ", tree: " << GetNewickSorted(false) << "     tree details: ";
//Dump();
    // reroot the subtee (rooted at nodeST) at a new root (nodeSTNewroot, a node in the subtee)
    // collect all the nodes above nodeSTNewRoot and their other children (not along the path from nodeST to ndoeSTNewRoot)
    vector<int> listAnces;
    int nodeCurr = nodeSTNewRoot;
    int nodeAnc = GetParent(nodeCurr);
    while( nodeAnc != nodeST )
    {
        YW_ASSERT_INFO( nodeAnc >= 0, "Fail" );
        int ch = GetLeftDescendant(nodeAnc);
        if( ch == nodeCurr )
        {
            ch = GetRightDescendant(nodeAnc);
        }
        listAnces.push_back(nodeAnc);
        
        nodeCurr = nodeAnc;
        nodeAnc = GetParent(nodeCurr);
    }
...
//cout << "List Ances: ";
//DumpIntVec(listAnces);
    
    // now reroot from the last
    int nodeNewChild = GetLeftDescendant( nodeST );
    if( nodeNewChild == *listAnces.rbegin()  )
    {
        nodeNewChild = GetRightDescendant(nodeST);
    }
    for(int i=(int)listAnces.size()-1; i>=0; --i)
    {
        int node1 = listAnces[i];
//cout << "Moving node1: " << node1 << ", nodeNewChild: " << nodeNewChild << endl;
        // set the other child to the
        SetParent( nodeNewChild, node1 );
        nodeNewChild = node1;
    }
    SetParent( listAnces[0], nodeST);
    SetParent( nodeSTNewRoot, nodeST);
//cout << "Before cleanup: tree: \n";
//Dump();
    // rearrange
    RearrangeParIncOrder();
...
//cout << "AFTER Reroot tree, tree: " << GetNewickSorted(false) << "     tree details: ";
//Dump();
}

void MarginalTree :: PerformDisjointSPRs(const vector<pair<int,int> > &listSPRs)
{
    if( listSPRs.size() == 0 )
    {
        return;
    }
    // perform multiple disjoint SPRs
    for(unsigned int i=0; i<listSPRs.size(); ++i )
    {
        int nodeSPRSrc = listSPRs[i].first;
        int nodeSPRDest = listSPRs[i].second;
        // perform tree rearrangement
        // cannot have root be involved in SPR
        if( nodeSPRSrc == GetRoot() || nodeSPRDest == GetRoot() )
        {
        ...
        {
            // donot allow SPR src and dest be siblings already
            cout << "WARNING: cannot have two siblings be SPR1\n";
            continue;
        }
        SetParent( nodeSPRSrcSib, GetParent(nodeSPRSrcPar), false );
        SetParent( nodeSPRDest, nodeSPRSrcPar, false );
...
void MarginalTree :: ConsDecedentLeavesInfo3( vector<set<int> > &descLaves ) const
{
    descLaves.clear();
    //vector< vector<int> > listDescendents;
    int numNodes = GetTotNodesNum();
    ...
    for(int i=0; i<GetNumLeaves(); ++i)
    {
        ...
        while( nodecur >= 0 )
        {
            descLaves[nodecur].insert(i);
            nodecur = GetParent(nodecur);
        }
    }
//cout << "Descedents info:\n";
//for( unsigned int i=0; i<descLaves.size(); ++i)
...
void MarginalTree :: ConsAllDescendantsInfo( vector<set<int> > &descNodes ) const
{
    descNodes.clear();
    //vector< vector<int> > listDescendents;
    int numNodes = GetTotNodesNum();
    ...
    for(int i=0; i<GetNumLeaves(); ++i)
    {
        ...
        while( nodecur >= 0 )
        {
            descNodes[nodecur].insert(nodecur);
            int nodepar = GetParent(nodecur);
            if( nodepar >= 0 )
            {
                UnionSets( descNodes[nodepar], descNodes[nodecur] );
            }
            nodecur = nodepar;
        }
    }
...
void MarginalTree :: ConsDecedentLeavesInfo2( map<set<int>, int> &mapSTToSTs ) const
{
    mapSTToSTs.clear();
    vector<set<int> > descLeaves;
    ConsDecedentLeavesInfo(descLeaves);
    for(unsigned int i=0; i<descLeaves.size(); ++i)
    {
        mapSTToSTs[ descLeaves[i] ] = i;
    }
}
void MarginalTree :: ConsDecedentLeavesInfo4( map<set<int>, int> &mapSTToSTs ) const
{
    mapSTToSTs.clear();
    vector<set<int> > descLeaves;
    ConsDecedentLeavesInfo3(descLeaves);
    for(unsigned int i=0; i<descLeaves.size(); ++i)
    {
        mapSTToSTs[ descLeaves[i] ] = i;
    }
...
void MarginalTree :: Dump() const
{
    ...
    cout << "Left descendents = ";
    DumpIntVec(listLeftDescs);
    cout << "Right descendents = ";
    DumpIntVec(listRightDescs);
}
...
string MarginalTree :: GetNewickAt(int node, bool fSort, bool fLen) const
{
	...
	if( childLeft < 0 )
	{
        ...
        if( fLen == true )
        {
            snprintf(buf, sizeof(buf), "%d:%0.4f", GetLabel(node), GetEdgeLen(node) );
        }
        else
        {
            snprintf(buf, sizeof(buf), "%d", GetLabel(node) );
        }
		res = buf;
	}
	else
	{
		...
		{
			char buf[100];
			snprintf(buf, sizeof(buf), ":%0.4f", GetEdgeLen(node) );
			res += buf;
		}
	}
	return res;
...
int MarginalTree :: GetSiblingFromDescInfo(int a, const vector<vector<int> > &descNodeInfo) const
{
    // get sibling of the node (leaf or non-leaf)
    int par = GetParent(a);
    int sib = -1;
    ...
    YW_ASSERT_INFO( par<(int) descNodeInfo.size(), "Wrong in GetSiblingFromDescInfo" );
    
    bool fFound = false;
    for(unsigned int ii=0; ii<descNodeInfo[par].size(); ++ii)
    {
        if( descNodeInfo[par][ii] != a)
        {
            sib = descNodeInfo[par][ii];
        }
        else
        {
            fFound = true;
    ...
    YW_ASSERT_INFO( fFound == true, "GetSiblingFromDescInfo: wrong" );
    return sib;
}
...
void MarginalTree :: CollectNumEdgesToAnces(std::map<int, std::map<int, int> > &mapNodeDistToAnces) const
{
    // return the number of edges from node to nodeAnces (its ancestor)
    // if nodeAnces is not the ancestor node of node, return -1
    mapNodeDistToAnces.clear();
    
    // init the root
    int root = GetRoot();
    mapNodeDistToAnces[root][root] = 0;
    
    // start from root and walk the way down
    for(int t=GetTotNodesNum()-1; t>=0; --t)
    {
        // get the children
        set<int> listChildren;
        GetChildren(t, listChildren);
        map<int,map<int,int> > :: iterator itPar = mapNodeDistToAnces.find(t);
        YW_ASSERT_INFO(itPar != mapNodeDistToAnces.end(), "Fail to find334");
        for(set<int> :: iterator it = listChildren.begin(); it != listChildren.end(); ++it)
        {
            int nodeChild = *it;
            mapNodeDistToAnces[nodeChild][nodeChild] = 0;
            // deal with all the ancestor of parent and plus one
            for(map<int,int> :: iterator itPar2 = itPar->second.begin(); itPar2 != itPar->second.end(); ++itPar2)
            {
                mapNodeDistToAnces[nodeChild][itPar2->first] = itPar2->second+1;
            }
        }
    }
}

void MarginalTree :: GetAllNodesBottomUpFrom(int nodeRoot, vector<int> &listNodes) const
{
    // get all nodes in the bottom up order, starting from the nodeRoot
    if( IsLeaf(nodeRoot) == false  )
    {
        GetAllNodesBottomUpFrom(GetLeftDescendant(nodeRoot), listNodes);
        GetAllNodesBottomUpFrom(GetRightDescendant(nodeRoot), listNodes);
    }
    listNodes.push_back(nodeRoot);
}
